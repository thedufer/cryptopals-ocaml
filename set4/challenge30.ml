...
let solve ~orig_hash ~orig_val verify =
  ...
  |> List.find_map ~f:(fun key_len ->
      let glue = String_util.md4_padding ~len:(key_len + String.length orig_val) in
      let new_hash =
        ...
        String_util.md4_seeded ~a ~b ~c ~d ~prefix_len:(key_len + String.length orig_val + String.length glue) new_message
      in
...
let%expect_test "md4" =
  String_util.md4 "" |> String_util.raw_to_hex |> print_endline;
  String_util.md4 "12345678901234567890123456789012345678901234567890123456789012345678901234567890"
